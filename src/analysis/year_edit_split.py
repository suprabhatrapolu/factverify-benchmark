...
Inputs: data/processed/sec_synthetic_test.parquet; output/predictions/<model>__sec_synthetic.csv.
Outputs: output/tables/year_edit_split.csv, docs/year_edit_split.md (the table row and the two sentences for the camera-ready)."""
...
for model, name in [("distilbert_full", "DistilBERT (full FEVER)"), ("distilbert", "DistilBERT (40k subset)"), ("xgboost", "XGBoost"), ("lexical_baseline", "Lexical baseline")]:
    pr = pd.read_csv(config.PREDICTIONS_DIR / f"{model}__sec_synthetic.csv").set_index("id")
...
out = pd.DataFrame(rows); out.to_csv(config.TABLES_DIR / "year_edit_split.csv", index=False)
...
(config.ROOT_DIR / "docs" / "year_edit_split.md").write_text("\n".join(md) + "\n")
